...
const GUIA_ITEMS = [
  {
    icon: '＋',
    titulo: 'Nueva tarea',
    desc: 'Haz clic en "+ Nueva tarea" (arriba a la derecha) para abrir el formulario de creación.',
    color: '#16a34a',
    bg: '#f0fdf4',
    border: '#bbf7d0',
  },
  {
    ...
    desc: 'Al crear una tarea puedes seleccionar un empleado en el campo inferior del formulario.',
    color: '#0369a1',
    bg: '#f0f9ff',
    border: '#bae6fd',
  },
  {
    icon: '🏷️',
    titulo: 'Estados',
    desc: 'Pendiente → tarea sin iniciar. En progreso → en ejecución. Completado → finalizada. Asignado → tiene empleado.',
    color: '#b45309',
    bg: '#fffbeb',
    border: '#fde68a',
  },
  {
    ...
    desc: 'Usa el botón ✕ al final de cada fila en la tabla para eliminar esa tarea.',
    color: '#dc2626',
    bg: '#fef2f2',
    border: '#fecaca',
  },
  {
    icon: '💰',
    titulo: 'Costo total',
    desc: 'Ingresa el costo estimado de la tarea. Es opcional y se muestra en la tabla principal.',
    color: '#7c3aed',
    bg: '#f5f3ff',
    border: '#ddd6fe',
  },
  {
    icon: '🔁',
    titulo: 'Recurrente',
    desc: 'Marca "Recurrente" si la tarea se repite periódicamente (ej. riego semanal, poda mensual).',
    color: '#0f766e',
    bg: '#f0fdfa',
    border: '#99f6e4',
  },
...
function AgrobotWidget() {
  ...
  const SUGERENCIAS = [
    ...
    '¿Puedo hacer tareas recurrentes?',
  ];
  ...
  return (
    <>
      <style>{`
        ...
        .aw-tabs {
          display: flex; border-bottom: 0.5px solid #e5e7eb;
        }
        .aw-tab {
        ...
        .aw-bubble {
          max-width: 82%; padding: 8px 12px; font-size: 13px; line-height: 1.5;
        }
        .aw-bubble.user {
              ...
              <div className="aw-chat-body">
                {mensajes.map((m, i) => (
                  <div key={i} className="aw-bubble-wrap">
                    <div className={`aw-bubble ${m.rol === 'user' ? 'user' : 'bot'}`}>
                      {m.texto}
                    </div>
                  </div>
                ))}
            ...
            <tbody>
              {tareas.map(t => (
                <tr key={t.idtarea}>
                  ...
                  <td><span className={`badge ${estColor[t.estado] ?? 'badge-yellow'}`}>{t.estado ?? '—'}</span></td>
