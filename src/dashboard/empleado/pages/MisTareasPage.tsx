...
export default function MisTareas({ usuario }: { usuario: UsuarioEmp }) {
  ...
  const cambiarEstado = async (id: number, nuevoEstado: Estado) => {
    ...
    try {
      if (nuevoEstado === "Completado") {
        // busca el idasigtarea de esta tarea para este empleado
        ...
        if (asignacion) {
          await fetch(
            `${API}/api/v1/tareas/${asignacion.idasigtarea}/completar`,
            {
              method: "PATCH",
            },
          );
  ...
  const pendientes = tareas.filter(
    (t) => normalizeEstado(t.estado) === "Pendiente",
  ).length;
  const enProgreso = tareas.filter(
    (t) => normalizeEstado(t.estado) === "En progreso",
  ).length;
  const completadas = tareas.filter(
    (t) => normalizeEstado(t.estado) === "Completado",
  ).length;

  const FILTROS: (Estado | "todas")[] = [
    "todas",
    "Pendiente",
    "En progreso",
    "Completado",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {tareasFiltradas.length === 0 ? (
        ...
        >
          No tienes tareas{" "}
          {filtro !== "todas"
            ? `con estado "${ESTADO_LABELS[filtro as Estado]}"`
            : "asignadas"}
        </div>
      ) : (
              ...
              >
                ...
                >
                  <span
                    style={{
                      fontSize: 14,
                      fontWeight: 600,
                      color: est === "Completado" ? "#9ca3af" : "#111",
                      textDecoration:
                        est === "Completado" ? "line-through" : "none",
                    }}
                  >
                    {tarea.tipoactividad ?? "—"}
                  </span>
                  <span
                    style={{ ...pill, ...ESTADO_COLORS[est], flexShrink: 0 }}
                  >
                    {ESTADO_LABELS[est]}
                  </span>
                </div>
                ...
                >
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        cambiarEstado(tarea.idtarea, e.target.value as Estado)
                      }
                      style={{
                        padding: "5px 10px",
                        border: "0.5px solid #e5e7eb",
                  ...
                  {/* botón observaciones */}
                  <button
                    onClick={() => toggleObs(tarea.idtarea)}
                    style={{
                      display: "flex",
                      alignItems: "center",
                      gap: 5,
                      ...
                      borderRadius: 8,
                      fontSize: 12,
                      background: obsAbiertas.has(tarea.idtarea)
                        ? "#f3f4f6"
                        : "#fff",
                      color: "#374151",
                      cursor: "pointer",
                ...
                {obsAbiertas.has(tarea.idtarea) && (
                  ...
                  >
                    ...
                    />
                    <button
                      onClick={() => guardarObs(tarea.idtarea)}
                      style={{
                        alignSelf: "flex-end",
                        padding: "6px 16px",
                        background: "#16a34a",
